  ...
  return <div className="bg-card border-b border-border">
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {summaryData.map((item, index) => <Card key={index} className={`border-2 ${item.borderColor} ${item.cardBg} shadow-custom-md hover:shadow-custom-lg hover:scale-105 transition-all duration-300 overflow-hidden`}>
              <CardContent className="p-6">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {item.subtitle && <p className="text-sm font-medium text-gray-500">
                        {item.subtitle}
                      </p>}
                  </div>
